...
router.get('/hospital/:id',authenticateHealthOrg, async (req, res) => {
    ...
    try {
      ...
      const hospital = await prismaClient.organization.findUnique({
        where: {
          id: Number(req.params.id), // Assuming `id` is an integer
        },
        select: {
          id: true,
          name: true,
